 ...
 */
 (function(){
	...
	if(!window.requestAnimationFrame) window.requestAnimationFrame = function(fn){return setTimeout(fn,15-Math.floor((+new Date)/1000)%116)};
...
(function($){
	...
	FlowGrid.prototype = {
		...
		_getNumCols : function(first_argument) {
			return Math.max(1, Math.min(2, Math.round($(window).width())));
		},
